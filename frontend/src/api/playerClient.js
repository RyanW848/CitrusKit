...
const playerClient = axios.create({
    baseURL: process.env.REACT_APP_DO_YOU_KNOW_BALL || 'https://do-u-know-ball.com',
    headers: {
...
export const getPlayerValues = async ({ budget, relevantStats, playersLeftToDraft, unavailablePlayers, players }) => {
    const body = {
        budget,
        relevant_stats: Array.isArray(relevantStats) ? relevantStats : relevantStats,
    };
    if (playersLeftToDraft != null)  body.players_left_to_draft = playersLeftToDraft;
    if (unavailablePlayers?.length)  body.unavailable_players   = unavailablePlayers;
